(function ($) {
  $(function () {
    ...
    */
    ...
    /* parallax */
    if ($('.js-ag-primary_parallax').length) {
      ...
      $(window).resize(function () {
        if (agDisplayWidth > '750') {
          if(!window.mobileAndTabletCheck()) {
            $('.js-ag-primary_parallax').parallax({
              xPosBG: '100%',
              yPosBG: 0,
              speedBG: -0.4,
    ...
    /* scroll animation */
    if ($('.js-anim_box').length) {
      function agScrollAnimation() {
        var agAnimItem = $('.js-anim_box'),
          agWindowHeight = $(window).outerHeight(),
          agOffsetCoefficient= .9;

        agAnimItem.each(function (i, elem) {
          if ($(window).scrollTop() >= $(elem).offset().top - agWindowHeight * agOffsetCoefficient - 100) {
            $(elem).find('.js-anim_content').addClass('js-ag-anim__active').removeClass('js-ag-anim__inactive');
          } else {
            $(elem).find('.js-anim_content').removeClass('js-ag-anim__active').addClass('js-ag-anim__inactive');
          }
        })
      }
